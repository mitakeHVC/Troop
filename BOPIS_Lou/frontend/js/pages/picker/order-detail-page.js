...
document.addEventListener('DOMContentLoaded', async () => {
   ...
   if (logoutButton) {
    ...
    try {
        ...
        if (pickingListTbody) {
            ...
            if (orderDetails.order_items && orderDetails.order_items.length > 0) {
                ...
                addItemStatusChangeListeners(orderDetails.id); // Call after rows are added
            } else {
                pickingListTbody.innerHTML = '<tr><td colspan="6" class="text-center">この注文に商品情報がありません。</td></tr>';
            }
        }
        ...
        if(readyForPickupBtn) {
            readyForPickupBtn.addEventListener('click', async () => {
                let allItemsHandled = true;
                document.querySelectorAll('.item-picking-status').forEach(selectElement => {
                ...
                if (!allItemsHandled) {
                    alert('すべての商品がピッキング済みまたは在庫なしとして処理されていません。');
                    return;
                }

                if (!confirm('この注文を準備完了としてマークしますか？')) {
                    return;
                }

                readyForPickupBtn.disabled = true;
                readyForPickupBtn.textContent = '処理中...';

                try {
                    await apiRequest(`/picker/orders/${orderId}/ready-for-pickup`, 'POST', null, true);
                    redirectTo(`complete.html?id=${orderId}`);
                } catch (error) {
                    console.error('Failed to mark order as ready for pickup:', error);
                    alert(`準備完了処理に失敗しました: ${error.message}`);
                    readyForPickupBtn.disabled = false;
                    readyForPickupBtn.textContent = '準備完了';
                }
            });
        }
    ...
    function addItemStatusChangeListeners(orderIdForAPI) {
        document.querySelectorAll('.item-picking-status').forEach(selectElement => {
            selectElement.addEventListener('change', async (event) => {
                ...
                try {
                    await apiRequest(`/picker/orders/${orderIdForAPI}/items/${itemId}`, 'PATCH', { picking_status: newStatus }, true);
                    event.target.dataset.originalValue = newStatus;
                } catch (error) {
                    console.error(`Failed to update status for item ${itemId}:`, error);
                    alert(`アイテム ${itemId} のステータス更新に失敗: ${error.message}`);
                    if (originalValue) event.target.value = originalValue;
                }
            });
